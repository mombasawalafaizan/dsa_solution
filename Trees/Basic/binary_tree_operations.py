...
class BinaryTree:
    ...
    """Traversals"""
    def inorder(self, node=None, recursive = True, morris = False):
        if morris:
            if node:
                BinaryTree.__morris_traversal(node)
            else:
                BinaryTree.__morris_traversal(self.root)
        elif recursive:
            if node:
        ...
        def helper(node, prev):
            if node:
                if helper(node.left, prev) == False:
                    # prev[0] = node.data
                    return False
                if prev[0] and prev[0] > node.data:
                    return False
                prev[0] = node.data
